...
class ILMref extends ILMtag {
	function __construct($parent,$follow=False) {
		ILMtag::__construct(0,$parent);
		$this->follow = $follow;
		$this->params = array();
	}
	function html($version=5,$t='') {
		...
		$o = explode('-',$this->params[0]);
		switch($p = array_shift($o)) {
		case 'mailto':
			$to = $this->params[2];
		...
		case 'doc':
			...
			break;
		case 'img':
			$x = explode('!',$this->params[2]);
			$r = isset($x[1]) ? $x[1]: $x[0];
			$t = empty($this->params[3])? Null: $this->params[3]; 
			$id = empty($o)? Null: array_shift($o);
			$cl = implode(' ',$o);
			$s = '<img src="'.$x[0].'"';
			if($t) $s.= " title=\"$t\" alt=\"$t\"";
			if($cl) $s.= " class=\"$cl\"";
			if($id) $s.= " id=\"$id\"";
			$s.= ">";
			break;
		...
		case 'ui':
			$r = '#';
			$s = empty($this->params[3])? '': $this->params[3];
			$s = '<i class="ui ui_'.$this->params[2].'">'.$s.'</i>';
			break;
		case 'style':
...
function ilm_tagcomp($m,$t,&$text) {
	$text = substr($text,strlen($m[0]));
	if(empty($t->pws) && empty($t->content)) {
		$t->add_class($t->name.'_'.$m[1]);
		if(!empty($m[2])) $t->pws = true;
	} else {
		$t->content.= $m[0];
	}
...
function ilm_link($m,&$t,&$text) {
	$text = substr($text,strlen($m[0]));
	if(empty($t->pws) && empty($t->content) && empty($t->id)) {
		array_pop($t->parent->tags);
		$t->parent->tags[] = $x = new ILMref($t->parent,True);
		#$x->copy($t);
		preg_match("#^([-\w]*)(:?)(.*)#",$m[1],$mm);
		array_shift($mm);
		$x->params = $mm;
...
function ilm_untag($m,&$t,&$text) {
	...
	#if(!empty($t->double)) {$pre = "/U ".$t->name;; $t = $t->parent; $pre = "-".$t->name." T/"; $text = $pre.$text;}
	$t = $t->parent;
}
